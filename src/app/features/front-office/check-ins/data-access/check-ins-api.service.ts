...
export class CheckInsApiService {
  ...
  getReservations():
    Observable<CheckInReservationList> {

    /*
     * The backend does not expose GET /check-ins.
     * This workspace therefore loads reservation data from the
     * verified GET /api/v1/reservations endpoint.
     *
     * The exact reservation list-query DTO was not available in
     * the retrieved source, so no unverified query parameters are
     * invented here.
     */
    return this.api
      .get<
  ...
  checkIn(
     ...
     * POST /api/v1/check-ins/{reservationId}
     *
     * Endpoint inventory shows reservationId is a route parameter.
     * No request-body DTO is verified, so an empty object is sent
