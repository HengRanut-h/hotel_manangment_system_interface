import {
  ChangeDetectionStrategy,
  Component,
  inject,
  signal
} from '@angular/core';

import {
  Router
} from '@angular/router';

import {
  getSafeApiErrorMessage
} from '../../../../../core/http/api-error.util';

import {
  TranslationPipe
} from '../../../../../core/i18n/translation.pipe';

import {
  TranslationService
} from '../../../../../core/i18n/translation.service';

import {
  ToastService
} from '../../../../../shared/ui/toast/toast.service';

import {
  GuestRequestFormComponent,
  GuestRequestFormValue
} from '../../components/guest-request-form/guest-request-form.component';

import {
  GuestRequestsApiService
} from '../../data-access/guest-requests-api.service';

@Component({
  selector:
    'app-guest-requests-create-page',

  standalone:
    true,

  imports: [
    TranslationPipe,
    GuestRequestFormComponent
  ],

  templateUrl:
    './guest-requests-create.page.html',

  styleUrl:
    './guest-requests-create.page.css',

  changeDetection:
    ChangeDetectionStrategy.OnPush
})
export class GuestRequestsCreatePage {

  private readonly api =
    inject(GuestRequestsApiService);

  private readonly router =
    inject(Router);

  private readonly toast =
    inject(ToastService);

  private readonly translation =
    inject(TranslationService);

  readonly submitting =
    signal(false);

  submit(
    value: GuestRequestFormValue
  ): void {

    if (this.submitting()) {
      return;
    }

    this.submitting.set(true);

    this.api
      .create({
        branchId:
          value.branchId,

        referenceNumber:
          value.referenceNumber,

        title:
          value.title,

        notes:
          value.notes,

        amount:
          value.amount,

        eventAtUtc:
          value.eventAtUtc,

        relatedEntityId:
          value.relatedEntityId,

        relatedEntityType:
          value.relatedEntityType
      })
      .subscribe({

        next:
          item => {

            this.submitting.set(false);

            this.toast.success(
              this.translation.translate(
                'guestRequests.createSuccess'
              )
            );

            void this.router.navigate([
              '/app/operations/guest-requests',
              item.id
            ]);
          },

        error:
          error => {

            this.submitting.set(false);

            this.toast.error(
              getSafeApiErrorMessage(
                error,
                this.translation.translate(
                  'guestRequests.createFailed'
                )
              )
            );
          }
      });
  }

  cancel(): void {

    if (this.submitting()) {
      return;
    }

    void this.router.navigate([
      '/app/operations/guest-requests'
    ]);
  }
}
